...
class Lead(models.Model):
    ...
    @api.depends('procurement_fee_rate', 'purchase_order_cif_amount', 'is_cif_override', 'manual_purchase_order_cif_amount')
    ...
    @api.depends('purchase_order_cif_amount', 'credit_cost_rate', 'credit_cost_month',  'is_cif_override', 'manual_purchase_order_cif_amount')
    ...
    @api.depends('supplier_delivery_method', 'purchase_order_cif_amount', 'procurement_commission_fob_amount',
                 ...
                 'road_transportation_amount', 'logistics_service_amount', 'is_internal',  'is_cif_override', 'manual_purchase_order_cif_amount')
    ...
    @api.depends('sales_cost', 'procurement_fee_amount', 'credit_cost_amount', 'initial_sales_price_unit',
                 'product_qty')
    ...
    @api.depends('sales_cost', 'procurement_fee_amount', 'credit_cost_amount', 'initial_sales_price',
    ...
    @api.depends('sales_price', 'total_cost', 'credit_cost_amount', 'is_sales_price_override', 'agreed_sales_price',
                 'total_cost_updated', 'total_cost_updated_agreed')
    def compute_markup_amount(self):
        for rec in self:
            if rec.is_internal:
                if rec.is_sales_price_override:
                ...
                if rec.is_sales_price_override:
                    rec.markup_amount = rec.agreed_sales_price - rec.total_cost_updated_agreed
                else:
                    rec.markup_amount = rec.sales_price - rec.total_cost_updated

    @api.depends('markup_amount', 'total_cost', 'total_cost_updated', 'is_sales_price_override',
                 'credit_insurance_amount', 'total_cost_updated_agreed')
        ...
        for rec in self:
            try:
                if rec.is_internal:
                    ...
                    if rec.is_sales_price_override:
                        rec.markup_percentage = rec.markup_amount / rec.total_cost_updated_agreed
                    else:
                        rec.markup_percentage = rec.markup_amount / rec.total_cost_updated
            except ZeroDivisionError:
