...
class PaymentRequest(models.Model):
    ...
    correspondent_bank_id = fields.Many2one('res.bank', related='bank_account_id.correspondent_bank_id',
                                            string="Correspondence Bank")
    ...
    payment_term_id = fields.Many2one('account.payment.term', related="purchase_order_id.payment_term_id",
    ...
    def _check_single_final_payment_per_purchase(self):
        for record in self:
            if (record.payment_category == 'supplier'
                    and record.type == 'final'
    ...
    def _compute_amount_words(self):
        for rec in self:
            ...
            rec.amount_words = currency.amount_to_text(rec.amount)
    ...
    def action_confirm(self):
        ...
        if not approvers:
            raise UserError(_("No approvers have been defined for this company."))
