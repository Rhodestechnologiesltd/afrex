...
class AccountMove(models.Model):
    ...
    sale_country_id = fields.Many2one('res.country', related='partner_id.country_id', string="Country of Delivery", store=True)
    ...
    breakbulk_container = fields.Selection([('breakbulk', "Breakbulk"),
                                            ('container', "Container"),],
                                           string="Breakbulk or Container", default='container', readonly=True, related='sale_order_id.breakbulk_container')
    
    container_type_id = fields.Many2one('asc.container.type', string="Container Size", readonly=True, related='sale_order_id.container_type_id')
    ...
    qty_delivered = fields.Float(string="MT Delivered", related="lead_id.purchase_order_qty_delivered", tracking=True, digits="Prices per Unit")
    ...
    stock_move_ids = fields.One2many('stock.move', 'sale_invoice_id', domain=[('state','=', ['assigned','done'])], string="Stock Moves")
    ...
    outgoing_doc_ids = fields.One2many('asc.document', 'sale_invoice_id', domain=[('type','=', 'outgoing')], string="Documents to be provided")
    incoming_doc_ids = fields.One2many('asc.document', 'sale_invoice_id', domain=[('type','=', 'incoming')], string="Documents to receive")
